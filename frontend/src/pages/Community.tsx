...
// // Example perks array
// const perks = [
//   { threshold: 1000, title: 'Bronze Saver', description: '5% discount on courses', color: 'from-sky-400 to-sky-600' },
//   { threshold: 2000, title: 'Silver Saver', description: '10% discount on courses', color: 'from-blue-500 to-blue-700' },
//   { threshold: 5000, title: 'Gold Champion', description: 'Free workshop coupon', color: 'from-emerald-500 to-emerald-700' }
// ];

// const formatLiters = (value: number) => `${new Intl.NumberFormat('en-IN').format(Math.round(value))} L`;
// const formatDate = (dateValue: string) => {
//   const parsed = new Date(dateValue);
//   if (Number.isNaN(parsed.getTime())) return dateValue;
//   return new Intl.DateTimeFormat('en-IN', { dateStyle: 'medium' }).format(parsed);
// };
// const getProgress = (value: number, goal: number) => (goal ? Math.min(Math.round((value / goal) * 100), 100) : 0);
...
//   // Fetch families with assessments
//   const fetchFamilies = async () => {
//     try {
//       const response = await axios.get<FamilyRecord[]>('http://localhost:5000/families');
//       const data = response.data;

//       // For each family, pick the assessment with the highest waterSaved
//       const updatedFamilies = data.map(family => {
//         if (!family.assessments || family.assessments.length === 0) return family;
//         const topAssessment = family.assessments.reduce((prev, curr) =>
//           curr.waterSaved > prev.waterSaved ? curr : prev
//         );
//         return { ...family, savings: { weekly: topAssessment.waterSaved, monthly: topAssessment.waterSaved, yearly: topAssessment.waterSaved } };
//       });

//       setFamilies(updatedFamilies);
//       if (!selectedFamilyId && updatedFamilies.length > 0) setSelectedFamilyId(updatedFamilies[0].id);
//     } catch (err) {
...
//   const leaderboard = useMemo(() => {
//     return [...families].sort((a, b) => b.savings[timeframe] - a.savings[timeframe]);
//   }, [families, timeframe]);
...
//         return acc;
//       },
//       { weekly: 0, monthly: 0, yearly: 0, goal: 0 }
//     );
//   }, [families, timeframe]);
...
//                 <p className="text-sm text-gray-500">Average Household Progress</p>
//                 <p className="text-xl font-semibold text-gray-800">
//                   {Math.round(communityTotals.goal ? (communityTotals.yearly / communityTotals.goal) * 100 : 0)}%
//                 </p>
//               </div>
//             </Card>
//             <Card className="flex items-center gap-4">
...
//         {/* Main content: Dashboard, Leaderboard, Perks, Family Profile */}
//         <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
//           <div className="xl:col-span-2 space-y-6">
//             {/* Water Saved Dashboard & families */}
//             {/* ... Keep your existing dashboard and family cards UI as-is ... */}
//             {/* Just the logic above ensures the highest water saved assessment is used for each family */}
//           </div>

//           <div className="space-y-6">
//             {/* Perks & Milestones */}
//             {/* ... Keep your perks card UI as-is ... */}

//             {/* Family Profile */}
//             {/* ... Keep your family profile UI as-is ... */}
//           </div>
//         </div>
//       </div>
//     </MainLayout>
