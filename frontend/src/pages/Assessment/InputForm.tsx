...
//   // ⚠️ New function to fetch state and district from coordinates
//   const fetchStateAndDistrict = async (latitude: number, longitude: number) => {
//     try {
//       const geoRes = await fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${latitude}&lon=${longitude}`);
//       const geoData = await geoRes.json();
      
//       const state = geoData.address.state || "";
//       const district = geoData.address.state_district || geoData.address.county || geoData.address.city || "";
...
//       setLocationError("Could not determine state and district from location.");
//     }
//   };


// // New function to fetch annual rainfall data by year
// const fetchAnnualRainfall = async (latitude: number, longitude: number) => {
//   setRainfallLoading(true);
//   setRainfallError(null);

//   try {
//     const lastFullYear = new Date().getFullYear() - 1; // e.g. if 2025 → use 2024
//     const startDate = `${lastFullYear}-01-01`;
//     const endDate = `${lastFullYear}-12-31`;

//     const url = `https://archive-api.open-meteo.com/v1/archive?latitude=${latitude}&longitude=${longitude}&daily=precipitation_sum&start_date=${startDate}&end_date=${endDate}&timezone=UTC`;
...
//       totalAnnualRainfall = data.daily.precipitation_sum.reduce(
//         (sum: number, value: number) => sum + value,
//         0
//       );
//     }

//     // Update state with the final annual total (in mm)
//     setFormData((prev) => ({
//       ...prev,
//       rainfall: totalAnnualRainfall.toFixed(2), // keep as string with 2 decimals
//     }));
//     setRainfallLoading(false);
//   } catch (error) {
//     console.error("Error fetching annual rainfall:", error);
//     setRainfallError("Failed to fetch annual rainfall data.");
//     setRainfallLoading(false);
//   }
// };
...
//         // ⚠️ Update lat and long in a single step
//         setFormData(prev => ({
//           ...prev,
...
//         // ⚠️ Call the new functions to get state, district, and rainfall
//         await fetchStateAndDistrict(latitude, longitude);
//         await fetchAnnualRainfall(latitude, longitude);
...
const AssessmentInput: React.FC = () => {
  ...
  // ⚠️ Corrected: Fetches state, district, and address from coordinates
  const fetchStateAndDistrict = async (latitude: number, longitude: number) => {
    try {
      const geoRes = await fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${latitude}&lon=${longitude}`);
      const geoData = await geoRes.json();
  ...
  const getCurrentLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        // ⚠️ Wait for both promises to resolve
        await Promise.all([
          fetchStateAndDistrict(latitude, longitude),
          fetchAnnualRainfall(latitude, longitude)
        ]);
  ...
  const handleNextStep = async () => {
    if (currentStep < 3) {
      ...
      try {
        ...
        // ⚠️ Fix 3: Save form data to localStorage before navigating
        ...
        // ⚠️ Redirect to the results page
        navigate("/results");
      } catch (error) {
        console.error("Failed to save assessment", error);
      }
