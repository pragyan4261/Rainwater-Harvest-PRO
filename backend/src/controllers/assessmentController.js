...
export const updateAssessment = asyncHandler(async (req, res) => {
    ...
    if (assessment) {
        ...
        assessment.dwellers = req.body.dwellers || assessment.dwellers;
        assessment.phone = req.body.phone || assessment.phone;
        assessment.email = req.body.email || assessment.email;
        assessment.roofArea = req.body.roofArea || assessment.roofArea;
        assessment.openSpace = req.body.openSpace || assessment.openSpace;
        assessment.roofType = req.body.roofType || assessment.roofType;
        assessment.soilType = req.body.soilType || assessment.soilType;
        assessment.address = req.body.address || assessment.address;
        assessment.state = req.body.state || assessment.state;
        assessment.district = req.body.district || assessment.district;
        assessment.latitude = req.body.latitude || assessment.latitude;
        assessment.longitude = req.body.longitude || assessment.longitude;
        assessment.rainfall = req.body.rainfall || assessment.rainfall;
...
export const deleteAssessment = asyncHandler(async (req, res) => {
    ...
    if (assessment) {
        // Check if the assessment belongs to the user or if user is an admin
        if (assessment.user.toString() !== req.user._id.toString() && !req.user.isAdmin) {
            res.status(401);
...
export const generatePdf = asyncHandler(async (req, res) => {
  ...
  const htmlContent = `
    ...
    <body>
      ...
      <p><strong>Efficiency:</strong> ${fullReport.efficiency || 'N/A'}%</p>
      <p><strong>Inertia:</strong> ${fullReport.inertia || 'N/A'}</p>
      <p><strong>Groundwater Level:</strong> ${full.groundwaterLevel || 'N/A'} meters</p>
